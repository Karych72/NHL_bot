...
| `psycopg2.errors.ForeignKeyViolation: … rosters_current_team_id_season_id_fkey` | Переименование франшизы: в полном историческом справочнике команд один `triCode` принадлежит двум `team_id` (Utah Hockey Club id=59 → Utah Mammoth id=68, оба «UTA»), и в `rosters.current_team_id` попадал id, которого нет в `teams` за этот сезон. `supplement_rosters_from_reports()` строит карту `triCode → team_id` по сезонным `teams`-строкам, а не по справочнику; если симптом вернулся — проверить, что в `teams` за сезон нет двух строк с одним `triCode`. |

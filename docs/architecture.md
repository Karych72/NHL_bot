...
(`_last_n_form_record()`), текущая серия (`_current_streak()`, Задача 24) и остальные
справочники/таблицы/лидеры в `bot_messages.py` кэшируются через `cached_fetch_all()`,
потому что меняются раз в загрузку данных.
